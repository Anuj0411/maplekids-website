...
import {
  ...
  loadSchoolContextForWhatsApp,
  formatSchoolContextForPrompt,
  saveConversationTurn,
  type SchoolContextForWhatsApp,
...
function mapIntentUserData(ctx: SchoolContextForWhatsApp): Record<string, unknown> {
  ...
  const total = a?.totalDaysRecorded ?? 0;
  ...
  return {
    ...
    feeStatus:
      'Fee balances are not stored in Firestore — parents should contact the school office for exact amounts and due dates.',
    presentDays: present,
    totalDays: total,
    attendancePercentage: pct,
    attendanceStatus: a?.lastStatus ?? (total === 0 ? 'No matching attendance rows yet' : 'See summary'),
    lastAttendanceDate: a?.lastRecordDate ?? 'n/a',
    latestTerm: 'Not stored in this assistant',
    subjectCount: 0,
    performance:
      'Official report cards are not loaded into this assistant — please contact the class teacher or office.',
  };
}
 ...
 */
export async function processMessage(message: any, value: any): Promise<void> {
  try {
    ...
    const schoolContext = await loadSchoolContextForWhatsApp(from, user);
    ...
    if (isGeminiConfigured()) {
      if (schoolContext.primaryStudent && STRUCTURED_INTENTS.has(intent)) {
        ...
        responseText = await generateIntentBasedResponse(intent, userData);
        await saveConversationTurn(from, messageText, responseText);
      } else {
        responseText = await generateAIResponse(messageText, from, {
          ...
          studentName: schoolContext.primaryStudent
            ? [schoolContext.primaryStudent.firstName, schoolContext.primaryStudent.lastName]
                ...
                .trim()
            : undefined,
          studentClass: schoolContext.primaryStudent?.class,
          schoolContextSummary: formatSchoolContextForPrompt(schoolContext),
        });
      }
    } else {
...
async function generateFallbackResponse(
  ...
  intent: string,
  schoolContext: SchoolContextForWhatsApp
): Promise<string> {
  const userName = user?.name || 'there';
  const summary = formatSchoolContextForPrompt(schoolContext);
  const attendanceLine =
    schoolContext.attendanceSummary && schoolContext.attendanceSummary.totalDaysRecorded > 0
